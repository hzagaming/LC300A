...
- 产品为 LC300A / 落川OS 300型，开发版本 `0.3.0-dev`，基于 Debian 13（trixie）amd64。
...
- `make test`：41 个单元测试、4 个 os-release fixture、bootstrap、清理、产品、品牌、桌面、安装器与仓库卫生契约通过。
- `make lint STRICT=1`：23 个 Shell/系统脚本、Python 配置校验、ShellCheck 与严格仓库检查通过。
- UI/UX：Logo、双主题壁纸、Plymouth、Plasma、SDDM 深色登录页、Calamares 欢迎/分区/用户/摘要/安装页面和至少 44 px 交互控件通过静态及真实帧缓冲检查。
...
- `make test-apps`：Konsole、Firefox、Discover、外网页面和 9.68 秒有效 QEMU 音频输出通过，峰值 7843、非零采样 300,788。
- `make test-installer`：完全断网安装、GPT、ext4、FAT EFI、`hwclock`、GRUB、安装后清理、移除 ISO 后启动、串口登录、SDDM 和 Plasma 通过。
- 当前 ISO 为 1,820,854,272 字节、1,617 个包，SHA-256 为 `00d5d01326a856d0dae417578bb401cce73cbd860b5f367226465e026a1a8b79`。
