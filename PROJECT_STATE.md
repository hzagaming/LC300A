...
阶段 4：桌面体验和系统应用（进行中）；首次登录欢迎体验与容量透明度增量完成。

## 当前事实

- 产品为 LC300A / 落川OS 300型，开发版本 `0.3.6-dev`，基于 Debian 13（trixie）amd64。
- 产品统一要求最低磁盘 16 GiB、推荐磁盘 32 GiB、最低内存 2 GiB、典型安装约 6 GiB；本轮展开 rootfs 实测 4,965,994,496 字节（约 4.63 GiB）。
...
- `make test`：59 个单元测试、4 个 os-release fixture、bootstrap、清理、产品、品牌、桌面、欢迎程序、安装器与仓库卫生契约通过。
- `make lint STRICT=1`：23 个 Shell/系统脚本、Python 配置校验、ShellCheck 与严格仓库检查通过。
- UI/UX：Logo、双主题壁纸、Plymouth、Plasma、SDDM 深色登录页、欢迎程序四页及 16/32/2/约 6 GiB 容量卡片、Firefox 单标签 HTTPS 正文、Discover 应用卡片，以及 Calamares 1080 px 生产界面的欢迎/分区/完整用户表单/摘要/安装/完成页面通过真实帧缓冲与人工审图。
...
- `make test-apps`：Konsole、Dolphin、系统设置、欢迎程序四页、Firefox 单标签 HTTPS 正文、Discover 真实应用卡片与 URI 动作通过；自动阶段捕获 6.87 秒有效 WAV，峰值 7,843、非零采样 150,394，且未检测到自动 BGM；手工环境音后总捕获 15.60 秒、非零采样 844,756。
...
- `make test-installer`：16 GiB/2 GiB 安装门槛配置、无调试参数的完整断网安装、GPT、ext4、FAT EFI、`hwclock`、GRUB、稳定完成页、安装后清理、移除 ISO 后启动、串口登录、SDDM、首次欢迎、`0600` 完成状态与不重复弹窗完整通过。
- 当前 ISO 为 1,808,596,992 字节、1,618 个包，SHA-256 为 `88513683b522bae7d2e13f32fd7c8e30502cd79137c47c99b540f45756b06054`。
